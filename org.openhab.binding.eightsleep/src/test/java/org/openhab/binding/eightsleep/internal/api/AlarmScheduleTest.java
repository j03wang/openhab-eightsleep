...
public class AlarmScheduleTest {
    ...
    public void disabledWeekdayAlarmStillComputesNextRun() {
        ...
        Instant now = ZonedDateTime.of(2026, 8, 22, 12, 0, 0, 0,
    ...
    public void weekendAlarmLandsOnAWeekendDay() {
        var alarm = alarm("07:00:00", true,
                "{\"saturday\":true,\"sunday\":true}");
        // Wednesday: next weekend day must be found deterministically
        Instant now = ZonedDateTime.of(2026, 8, 26, 12, 0, 0, 0,
                ZoneId.of("America/Los_Angeles")).toInstant();
        Instant next = alarm.computeNextRun(ZoneId.of("America/Los_Angeles"), now);
        assertNotNull(next);
        ZonedDateTime zdt = next.atZone(ZoneId.of("America/Los_Angeles"));
        String dow = zdt.getDayOfWeek().toString();
        assertTrue("must be SATURDAY or SUNDAY, was " + dow,
                "SATURDAY".equals(dow) || "SUNDAY".equals(dow));
    }
    ...
    public void oneShotFallsBackToServerTimestamp() {
        var alarm = alarm("09:00:00", true, null);
        alarm.nextTimestamp = "2026-08-30T16:00:00Z";
    ...
    public void weekdayTakesOverAfterWeekendSlotPasses() {
        ...
        Instant now = ZonedDateTime.of(2026, 8, 23, 8, 0, 0, 0,
                ZoneId.of("America/Los_Angeles")).toInstant();
        ...
        assertTrue("weekday Mon must be sooner than weekend's next Sat",
                weekdayNext.isBefore(weekendNext));
    }

    /** Malformed time strings yield null instead of throwing. */
    @Test
    public void malformedTimeYieldsNull() {
        Instant now = Instant.parse("2026-08-22T12:00:00Z");
        assertNull(alarm("not-a-time", true, null).computeNextRun(ZoneId.of("UTC"), now));
