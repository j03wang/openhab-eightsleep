 ...
 */
package org.openhab.binding.eightsleep.internal.temperature;
...
public class HeatingLevelConversionTest {
    ...
    public void nonFiniteTemperatureMapsToNeutralLevel() {
        ...
        assertEquals(27.0, HeatingLevelConversion
                .levelToTemperature(HeatingLevelConversion.temperatureToLevel(Double.NaN, false), false), EPS);
    }
    ...
    public void celsiusAndFahrenheitTablesAgree() {
        for (int level = -100; level <= 100; level += 10) {
            ...
            assertTrue("unit tables disagree at level " + level, Math.abs(f - (c * 1.8 + 32)) < 2.5);
        }
    }
}
